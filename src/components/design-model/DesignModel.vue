...
<template>
    ...
    <aside class="card rounded-0 left">
      <div class="card-header">
        <h4 class="card-title d-flex align-items-center"><img src="@/assets/icon-workflow.svg" alt="icon-workflow" class="me-2"> Workflow</h4>
      </div>
      <div class="card-body">
        <div class="bg-dark p-4 border-radius">
          ...
          <ul class="nav-workflow ">
            ...
            <button @click="setDesignModelStep('READOUT_PARAMETERS')">Readout parameters</button>
            <button @click="setDesignModelStep('INITIALIZATION_PARAMETERS')">Initialization parameters</button>
          </ul>

          <ul class="nav-workflow ">
            <span class="headline">Specify Bath</span>
            <button @click="setDesignModelStep('BATH_TYPE')">Bath type</button>
            <button @click="setDesignModelStep('BATH_PARAMETERS')">Bath parameters</button>
            <button @click="setDesignModelStep('SYSTEM_BATH_COUPLING')">System-bath coupling</button>
          </ul>
        </div>
        ...
        <div class="bg-dark p-4 border-radius ">
         ...
         <div class="d-flex-gap align-items-center">
          <div class="form-group d-flex flex-column">
            ...
            <input type="number" class="form-control" style="max-width:6rem" :value="selectedQudit.frequency"/>
          </div>
          <div style="font-size:20px">&#xB1;</div>
          <div class="form-group  d-flex flex-column">
            <label class="form-label">Variation</label>
            <div class="d-flex-gap align-items-center">
              <input type="number" class="form-control" style="max-width:6rem" :value="selectedQudit.variation"/>GHz
            </div>
          </div>
         </div>
          ...
          <div class="form-group d-flex flex-column justify-content-start">
            ...
            <div class="d-flex-gap align-items-center">
              <input type="number" class="form-control" :value="selectedQudit.anharmonicity"/>GHz
            </div>
          </div>
        </div>
      ...
      <div class="card-body">
        <div class="bg-dark p-4 border-radius">
          ...
          <div class="d-flex-gap align-items-center">
            <div class="form-group d-flex flex-column">
              <label class="form-label">Frequency</label>
              <input type="number" class="form-control" style="max-width:6rem" :value="selectedCoupling.frequency"/>
            </div>
            <div style="font-size:20px">&#xB1;</div>
            <div class="form-group  d-flex flex-column">
              <label class="form-label">Variation</label>
              <div class="d-flex-gap align-items-center">
                <input type="number" class="form-control" style="max-width:6rem" :value="selectedCoupling.variation"/>GHz
              </div>
            </div>
          </div>
          <div class="p-4">
          ...
          <div v-for="(constant, index) in selectedCoupling.constants" class="constant" :key="index">
            <span class="mb-2">H<sub>15</sub> =0.01 | 10 <img src="@/assets/icon-insert-constant.svg" alt=""> 01 |</span>
          </div>
    ...
    <!-- Readout parameters  -->
    <aside class="card rounded-0 card right" v-if="designModelStep === 'READOUT_PARAMETERS'">
      <div class="card-header d-flex justify-content-between" style="padding-top: 1rem; padding-bottom: 1rem">
        <h4 class="card-title d-flex align-items-center">Qudit</h4>
        <button class="btn">Delete Qudit</button>
      </div>
    ...
    <aside class="card rounded-0 card right" v-if="designModelStep === 'INITIALIZATION_PARAMETERS'">
      <div class="card-header d-flex justify-content-between" style="padding-top: 1rem; padding-bottom: 1rem">
        <h4 class="card-title d-flex align-items-center">Qudit</h4>
        <button class="btn">Delete Qudit</button>
      </div>
    ...
    <div class="modal-backdrop" v-if="designModelStep === 'BATH_TYPE'">
      <div class="modal-content">
        ...
        <button class="btn" @click="setDesignModelStep('BATH_PARAMETERS')">Continue</button>
      </div>
    </div>

    <!-- BATH PARAMETERS  -->
    <aside class="card rounded-0 card right" v-if="designModelStep === 'BATH_PARAMETERS'">
      <div class="card-header d-flex justify-content-between" style="padding-top: 1rem; padding-bottom: 1rem">
        <h4 class="card-title d-flex align-items-center">Qudit</h4>
        <button class="btn">Delete Qudit</button>
      </div>
